            ...
            for each_plot_line in each_plot:
                ...
                for x in tmpstr:
                    ...
                    if x=='P':
                        ...
                        xls[shtName].cell(row=iRow + 1, column=iColumn + 1).fill = fill
                    if str(xls[shtName].cell(row=iRow + 1, column=(totalsiteCnt*interval_columns) +(iColumn-(siteCnt-1)*interval_columns)+ 1).value)!='None':
                        y=str(xls[shtName].cell(row=iRow + 1, column=(totalsiteCnt*interval_columns)+(iColumn-(siteCnt-1)*interval_columns) + 1).value)+x
                        xls[shtName].cell(row=iRow + 1, column=(totalsiteCnt*interval_columns)+(iColumn-(siteCnt-1)*interval_columns) + 1, value=y)
                        if 'P' in y and '.' in y:
                            fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type="solid", )
                            xls[shtName].cell(row=iRow + 1, column=(totalsiteCnt*interval_columns)+(iColumn-(siteCnt-1)*interval_columns) + 1).fill = fill
                    else:
                        xls[shtName].cell(row=iRow + 1, column=(totalsiteCnt * interval_columns) + (iColumn-(siteCnt-1)*interval_columns) + 1, value= x)
                    iColumn = iColumn + 1
                iRow = iRow + 1
                pass
